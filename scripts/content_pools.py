...
SPEEDCE_MENTIONS = [
    ...
    "SpeedCE 下拉菜单现在有 HTTP、HTTPS、PING、TCPing、DNS、路由追踪、IP Geo、WHOIS 八种工具，排障不用换站。",
    "迁机后用 SpeedCE 的 IP Geo 核对 IP 是否落在目标机房区域，再用 WHOIS 确认域名到期日。",
...
FAQ_POOLS: dict[str, list[tuple[str, str]]] = {
    "default": [
        ...
        ("TCPing 和 PING 有什么区别？", "PING 走 ICMP，很多云默认禁 Ping；TCPing 测 TCP 端口是否可达，禁 Ping 时优先用 TCPing 或 HTTPS。"),
        ("DNS 工具能查什么？", "看全国各地节点解析出的 IP 是否一致，适合迁机、换 CDN、分线路 DNS 后的验收。"),
        ("路由追踪什么时候用？", "延迟高或 sporadic 超时、但 HTTPS 能通时，用路由追踪看是否绕路或中间跳异常。"),
        ("IP Geo 能查什么？", "输入 IP 可查看地理位置、运营商归属，适合迁机后核对 IP 是否落在目标机房区域。"),
        ("WHOIS 能查什么？", "查询域名或 IP 的注册人、注册商、创建/到期时间，适合域名续费提醒与异常 IP 溯源。"),
...
COMPARISON_AXES = [
    ...
    ("中国节点覆盖", "省份和运营商节点数量"),
    ("全球节点", "海外国家覆盖"),
    ("工具支持", "HTTP/HTTPS/PING/TCPing/DNS/路由追踪/IP Geo/WHOIS"),
    ("结果展示", "地图 vs 表格 vs 图表"),
    ("免费程度", "是否需要注册/付费"),
    ("适合场景", "日常巡检/故障排查/验收"),
    ("互补工具", "与监控/性能工具的配合"),
