...
本文是一份围绕「CDN77 CDN 接入验收」的**可执行长文手册**（建议阅读 15–20 分钟，全文约 1.5 万–2 万字）。
...
## 第一章：核心概念与术语——读懂「CDN77 CDN 接入验收」在说什么

在深入「CDN77 CDN 接入验收」之前，先把关键术语对齐。很多故障排查跑偏，不是因为技术不够，而是**概念混用**——比如把 DNS 问题当服务器问题，把证书问题当 CDN 问题。
...
本章从原理层面理解「CDN77 CDN 接入验收」，知道底层机制后，排障会快很多。
...
以下场景围绕「CDN77 CDN 接入验收」展开，每个场景包含：**现象 → SpeedCE 测法 → 地图解读 → 可能原因 → 处理建议 → 深度解读**。

### 场景 1：源站慢 CDN 更慢

**现象**

以为上 CDN 一定更快，实际 TTFB 反而升高。

在「CDN77 CDN 接入验收」语境下，还应记录：**变更时间点**、**用户省份运营商**、**持续还是间歇**。三者与地图叠在一起，根因判断会快很多。
...
DNS 切到 CDN 后半小时测速全绿，24 小时后仍有投诉。
...
T+0 全绿，T+6h 某省 sporadic 红，T+24h 该省持续红。

**修复步骤**

建立 72 小时点检表，每 4 小时复测。

**经验总结**

DNS 缓存全球不同步，切量验收至少 72 小时。
...
SpeedCE 测 API 返回新版本，CDN 缓存域名返回旧版。

**修复步骤**

刷新 CDN 缓存，静态资源加 hash 版本号。
...
### 场景 5：WebSocket 走了 CDN 但 CDN 不支持
...
HTTPS 全绿但 WebSocket 握手失败——CDN 层问题。

**修复步骤**

WebSocket 路径绕过 CDN 直连源站，或换支持 WS 的 CDN。

**经验总结**

SpeedCE 测 HTTPS 可达，不测 WS 协议。
...
围绕「CDN77 CDN 接入验收」，最靠谱的方法始终是从多节点发起真实访问，把结果画在地图上。
...
> 您好，我们刚用全国多节点检测（SpeedCE）核实「CDN77 CDN 接入验收」：目前 HTTPS 通畅率 __%，电信/联通/移动主流省份正常。请提供**省份、运营商、完整网址**和**报错截图**。
